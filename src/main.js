...
if (!reduced && innerWidth > 700) {
  ...
  const glow = document.querySelector('.hero-glow'); const hero = document.querySelector('.hero'); const destinationMap = document.querySelector('.hero-destinations'); addEventListener('mousemove', e => { glow.style.transform = `translate(${(e.clientX / innerWidth - .5) * 40}px, ${(e.clientY / innerHeight - .5) * 40}px)`; }); if (hero && destinationMap) { let destinationTimer; hero.addEventListener('pointermove', e => { const rect = hero.getBoundingClientRect(); const x = (e.clientX - rect.left) / rect.width - .5; const y = (e.clientY - rect.top) / rect.height - .5; destinationMap.style.setProperty('--route-x', `${x * 22}px`); destinationMap.style.setProperty('--route-y', `${y * 16}px`); destinationMap.classList.add('is-engaged'); clearTimeout(destinationTimer); destinationTimer = setTimeout(() => destinationMap.classList.remove('is-engaged'), 850); }); hero.addEventListener('pointerleave', () => { destinationMap.style.setProperty('--route-x', '0px'); destinationMap.style.setProperty('--route-y', '0px'); destinationMap.classList.remove('is-engaged'); }); }
